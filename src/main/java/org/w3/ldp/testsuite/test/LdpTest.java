...
public abstract class LdpTest implements HttpHeaders, MediaTypes {
	 ...
	 */
	protected boolean hasLinkHeader(Response response, String uri,
			String linkRelation) {
		List<Header> linkHeaders = response.getHeaders().getList(LINK);
		for (Header h : linkHeaders) {
			// The following code requires JAX-RS 2.0, but I'm not sure we want
			// that
			...
			// }
			// }
			// FIXME: This is not a proper test. Need to find a library to parse
			// the link header.
			if (h.getValue().contains(uri)
					&& h.getValue().contains(linkRelation)) {
				return true;
			}
		}
	...
	public Model getResourceAsModel(String uri, String mediaType) {
		return RestAssured.given().header(ACCEPT, mediaType).expect()
				.statusCode(HttpStatus.SC_OK).when().get(uri)
