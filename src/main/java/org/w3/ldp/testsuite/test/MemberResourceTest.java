...
public class MemberResourceTest extends RdfSourceTest {
	...
	@Parameters({ "memberResource", "directContainer", "indirectContainer",
			"basicContainer", "post" })
	public MemberResourceTest(@Optional String memberResource,
			@Optional String directContainer,
			@Optional String indirectContainer,
			@Optional String basicContainer, @Optional String post) {
		// If resource is defined, use that. Otherwise, fall back to creating
		// one from one of the containers.
		...
		if (this.memberResource == null) {
			...
			ResponseSpecification reSpec = RestAssured.given()
					.contentType(TEXT_TURTLE)
					.body(model, new RdfObjectMapper()).expect()
					.statusCode(HttpStatus.SC_CREATED)
					.header(LOCATION, notNullValue()).when();

			Response postResponse = reSpec.post(this.container);
	...
	public void deleteTestResource() {
		// If container isn't null, we created the resource ourselves. To clean
		// up, delete the resource.
		if (container != null) {
			RestAssured.expect().statusCode(isSuccessful()).when()
					.delete(memberResource);
		}
	}
